...
import { Search, TrendingUp, TrendingDown, Users, DollarSign, ArrowUpDown, Filter, RefreshCw, ChevronDown, ChevronUp, ExternalLink, ListPlus } from "lucide-react";
          ...
          <>
            ...
            <div className="flex justify-end mb-2">
              <button
                type="button"
                onClick={() => {
                  const r = bulkAddToWatchlist(stocks.map(s => ({ ticker: s.ticker, name: s.name })), "screener");
                  window.alert(`Watchlist: ${r.added} neu, ${r.skipped} übersprungen`);
                }}
                className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] border border-border/50 text-foreground/80 hover:bg-muted/40 disabled:opacity-40"
          ...
          <button onClick={onToggle} className="flex items-center gap-2 group">
            <div>
              ...
              <div className="mt-0.5" onClick={(e) => e.stopPropagation()}>
                <TickerAddButtons ticker={s.ticker} name={s.name} source="screener" compact />
              </div>
            </div>
