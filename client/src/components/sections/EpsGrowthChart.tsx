...
export default function EpsGrowthChart({ data }: { data: StockAnalysis }) {
  ...
  const epsHistory = pc?.epsHistory;
  const peerAvgEpsHistory = pc?.peerAvgEpsHistory;
  const sectorMedian = pc?.sectorMedian;
  // null when sector growth is missing — no fabricated 10% projection
  const sectorGrowth = sectorMedian?.epsGrowth ?? null;

  // Build unified chart data: merge subject, peer avg, sector projected line
  // (hook runs unconditionally — early return comes after, per Rules of Hooks)
  const chartData = useMemo(() => {
    if (!epsHistory || epsHistory.length < 3) return [];
    ...
    return filtered.map(year => {
      ...
      const sectorProjectedEps = sectorGrowth != null
        ? +(baseEps * Math.pow(1 + sectorGrowth / 100, yearsSinceBase)).toFixed(2)
        : null;
  ...
  if (!epsHistory || epsHistory.length < 3 || chartData.length < 3) return null;
  ...
  // Growth rates for display — CAGR only meaningful with positive start and end EPS
  const safeCAGR = (endEps: number, startEps: number, years: number): number | null => {
    if (startEps <= 0 || endEps <= 0 || years <= 0) return null;
    const v = +(((endEps / startEps) ** (1 / years) - 1) * 100).toFixed(1);
    return Number.isFinite(v) ? v : null;
  };
  const hist = epsHistory.filter(h => !h.isEstimate);
  const histGrowth = hist.length >= 2
    ? safeCAGR(hist[hist.length - 1].eps, hist[Math.max(0, hist.length - 6)].eps, Math.min(5, hist.length - 1))
    : null;
  const est = epsHistory.filter(h => h.isEstimate);
  const estGrowth = est.length >= 2 && hist.length > 0
    ? safeCAGR(est[est.length - 1].eps, hist[hist.length - 1].eps, est.length)
    : null;
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {histGrowth != null && (
          <div className="flex items-center gap-1 px-2 py-1 rounded bg-primary/10 border border-primary/20">
            ...
            <span className={`text-[11px] font-mono font-semibold ${sectorGrowth != null && histGrowth > sectorGrowth ? "text-emerald-400" : "text-foreground/70"}`}>
        ...
        {estGrowth != null && (
          <div className="flex items-center gap-1 px-2 py-1 rounded bg-primary/5 border border-primary/10">
            ...
            <span className={`text-[11px] font-mono font-semibold ${sectorGrowth != null && estGrowth > sectorGrowth ? "text-emerald-400" : "text-foreground/70"}`}>
        ...
        <div className="flex items-center gap-1 px-2 py-1 rounded bg-amber-500/5 border border-amber-500/10">
          ...
          <span className="text-[11px] font-mono font-semibold text-amber-400/80">
            {pc?.peerAvg.epsGrowth5Y != null
              ? `${pc.peerAvg.epsGrowth5Y >= 0 ? "+" : ""}${pc.peerAvg.epsGrowth5Y.toFixed(1)}%`
              : "—"}
          </span>
        </div>
        {sectorGrowth != null && (
          <div className="flex items-center gap-1 px-2 py-1 rounded bg-foreground/5 border border-foreground/10">
            ...
            <span className="text-[11px] font-mono text-foreground/50">{sectorGrowth >= 0 ? "+" : ""}{sectorGrowth}%</span>
          </div>
        )}
      </div>
