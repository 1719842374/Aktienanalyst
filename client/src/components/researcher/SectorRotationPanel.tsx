        ...
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={chartData} outerRadius="70%">
            {/* Bugfix (Nutzer-Feedback, schwer lesbare Achsenlabels): CSS-Variablen
                sind reine HSL-Komponenten ("222 47% 11%"), kein gueltiger CSS-Farbwert
                ohne hsl()-Wrapper -- der Browser/SVG-Renderer fiel dadurch auf naeherungs-
                weise Schwarz zurueck, kaum lesbar auf dem dunklen Panel-Hintergrund.
                Gleiches Muster wie an anderer Stelle im Projekt (GoldMonteCarloSection.tsx)
                korrekt verwendet: hsl(var(--foreground)) statt var(--foreground). */}
            ...
            <tbody>
              {(data.sectors || []).map(s => (
                <tr key={s.id} className="border-b border-border/20 text-[11px]">
                  ...
                  <td className="px-3 py-3 min-w-[200px] overflow-x-hidden" data-testid={`cycle-progress-row-${s.id}`}>
