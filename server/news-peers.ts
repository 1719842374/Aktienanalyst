 ...
 * RESTORED 15.08.2026 from 029df86 + WORK_PEER_ROIC_SANITY (ROIC_ABS_CAP, market-cap band).
 */
...
function isIndustryCompatible(
  subjectSector: string, subjectIndustry: string, candidateSector: string, candidateIndustry: string
  ...
  if (nSubjInd && nCandInd && nSubjInd === nCandInd) return { ok: true, reason: "Exact Industry-Match" };
  if (nSubjSec && nCandSec && nSubjSec === nCandSec) return { ok: true, reason: "Sector-Match" };
...
export async function filterAndSelectPeers(
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (!prof) { console.log(`[PEERS] ${upperSubject}: ${sym} verworfen — Profil nicht abrufbar`); continue; }
    const check = isIndustryCompatible(subjectSector, subjectIndustry, prof.sector, prof.industry);
    if (check.ok) filtered.push(sym);
    else console.log(`[PEERS] ${upperSubject}: ${sym} verworfen — ${check.reason}`);
  }
  ...
  if (curated) {
    ...
    console.log(`[PEERS] ${upperSubject}: kuratierte NEV-Peer-Liste verwendet (${curated.length} kuratiert)`);
...
export function extractRoicFromKeyMetricsRows(rows: any[]): RoicPoint {
  ...
  const roic5YPercent = yearValues.length >= MIN_ROIC_5Y_YEARS
    ? +(yearValues.reduce((a, b) => a + b, 0) / yearValues.length).toFixed(1) : null;
  return {
    roicPercent,
...
export async function fetchPeerComparison(
  ...
  try {
    ...
    console.log(`[PEERS] Failed for ${ticker}: ${err?.message?.substring(0, 200)}`);
    return null;
  }
}
