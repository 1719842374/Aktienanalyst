...
export function registerLiquidityRoute(app: Express): void {
  app.get("/api/researcher/liquidity", async (req, res) => {
    ...
    if (force) {
      deleteCache(tab, cacheParams);
      console.log("[RESEARCHER/liquidity] cache invalidated (refresh/force)");
    }
    if (!force) {
      const cached = readCache(tab, cacheParams);
      if (cached) {
        console.log(`[RESEARCHER/liquidity] cache HIT age=${cached._cacheAge}min`);
