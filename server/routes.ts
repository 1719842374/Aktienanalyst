...
export async function registerRoutes(server: Server, app: Express) {
  ...
  const analyzeHandler = async (req: any, res: any) => {
    ...
    try {
      ...
      // Forward EPS growth (moved up from the Lynch-classification block below —
      // must be declared before use in _pegGrowth to avoid a TDZ ReferenceError).
      ...
      // (epsGrowthFwd already computed above, before _pegGrowth)
      ...
      // === Growth thesis prefix (sector-hybrid warning) ===
      // (moved up from below the growthThesis block — must be declared before use)
      const hybridPrefix = sectorHybridNote ? `⚠️ ${sectorHybridNote} ` : "";

      // === Capex Fiscal Tailwind Lookup (runs before growthThesis/capexCtxForThesis) ===
      // (moved up from below — analyzeCapexContext must be declared before capexCtxForThesis uses it)
      ...
      // === Moat rating ===
      // (hybridPrefix + analyzeCapexContext lookup moved above — declared before growthThesis block uses them)
